...
`consts.data_view_id` is available in each workflow as an override if you want to point it at a
different existing data view instead of the one derived from `consts.index`.
...
- `gti-soar-ip-relationship` shares one index across GTI's 22 relationship object types, and genuinely
  ...
  under `gti` for every item, colliding or not. See `gti-workflow-design-decisions.md`'s "Index
  bootstrapping" section for the full reasoning and how to reconsider it per action.
...
reports a hash has. A large iteration count (250 pages, `consts.max_pages_safety`) exists purely as a
